window.VikaBacktest=(function(){
  ...
  function addPrediction(p){const a=read();const row={id:p.id||`${Date.now()}-${Math.random().toString(36).slice(2,7)}`,createdAt:new Date().toISOString(),sport:p.sport,selection:p.selection,probability:Number(p.probability),fair:p.fair??null,odds:p.odds??null,status:'OPEN',result:null,won:null};a.push(row);write(a);return row}
