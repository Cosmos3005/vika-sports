window.VikaRadar=(function(){
  ...
  function model(e,sport,ranks){
    ...
    const prediction=window.VikaAIEngine.calculate({sport,home:h,away:a,stats:null,mode});
    if(!prediction)return null;
    const line=odds(c.odds?.[0]||e.odds?.[0])[prediction.selection];
    const quality=Number(prediction.quality||0),readiness=quality>=2?3:Math.min(2,quality+1);
    const signal=window.VikaSignalEngine.evaluate({prediction,odds:line,dataReadiness:readiness,quality,conflict:false,mode});
    return{eventId:e.id,sport,mode,home:h.team?.displayName||'Home',away:a.team?.displayName||'Away',time:e.date,selection:prediction.selection,probability:prediction.probability,fair:prediction.fair,odds:line??null,edge:signal.edge??null,confidence:prediction.confidence,status:signal.status==='SIGNAL'?'SIGNAL':'WATCH',reason:signal.reason,modelReason:prediction.reason};
  }
  function draw(rows,meta){const box=document.querySelector('#radarResults'),metaBox=document.querySelector('#radarMeta');if(!box)return;const signals=rows.filter(x=>x.status==='SIGNAL'),watch=rows.filter(x=>x.status==='WATCH');if(metaBox)metaBox.textContent=`Последний скан: ${new Date().toLocaleTimeString('ru-RU',{hour:'2-digit',minute:'2-digit'})} · проверено ${meta.scanned} · SIGNAL ${signals.length} · WATCH ${watch.length}`;if(!rows.length){box.innerHTML='<div class="radar-empty">Пока нет матчей, которые можно корректно проверить. Vika ничего не выдумывает.</div>';return}box.innerHTML=rows.slice(0,10).map(x=>{const edge=x.edge==null?'нет линии':`${x.edge>=0?'+':''}${(x.edge*100).toFixed(1)}%`;const q=x.selection==='home'?x.home:x.selection==='away'?x.away:'Ничья';const href=`match.html?sport=${encodeURIComponent(x.sport)}&eventId=${encodeURIComponent(x.eventId)}&home=${encodeURIComponent(x.home)}&away=${encodeURIComponent(x.away)}&mode=${encodeURIComponent(x.mode)}`;return `<article class="radar-row"><div><span class="radar-status ${x.status.toLowerCase()}">${x.status==='SIGNAL'?'🔥 SIGNAL':'👁 WATCH'}</span><b>${x.home} · ${x.away}</b><small>${q} · ${LABEL[x.sport]||x.sport} · ${x.mode==='live'?'LIVE':'PREMATCH'}</small></div><div class="radar-metrics"><span>AI <b>${x.probability.toFixed(1)}%</b></span><span>Fair <b>${x.fair.toFixed(2)}</b></span><span>Edge <b>${edge}</b></span><span>Conf <b>${x.confidence.toFixed(0)}%</b></span></div><a href="${href}">Открыть →</a></article>`}).join('')}
